...
- 离线可演示：没有 LLM API Key 时也能完整跑通；配置 OpenRouter 后可增强问答和清洗效果。
...
- OpenRouter/OpenAI-compatible API Key，用于 LLM 增强问答和 Markdown 清洗。
...
按需编辑 `D:\Projects\EL\text_archiver\.env`，配置 OpenRouter 或 OpenAI-compatible API。
...
6. 查看个人学习进度统计。
7. 后续 Stage 3 会将“申请贡献到公共库”从占位能力升级为真实审核闭环。
...
- 明确私人空间、贡献空间和公共知识库的数据边界。
- 用户资料默认 private。
- 用户可主动申请贡献笔记、题解、错题和讲解。
